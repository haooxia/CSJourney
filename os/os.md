...
4. Linux内核设计理念：
   1. MultiTasK
      ...
      2. 并行parallelism：对于多核 CPU 时，多个任务可以同时被不同核心的 CPU 同时执行。
         ...
         2. 进程数量>cpu核数时，进程将时分复用cpu资源(并发)，但cpu极快，产生并行的错觉
...
1. 先来先服务（First Come First Served, FCFS）(非抢占式)：对长作业有利，适合cpu繁忙型作业的系统，不适合io繁忙型作业的系统。
...
0. **死锁deadlock**：多个进程因竞争资源而造成的一种僵局，没有外力作用，都无法继续执行。
   1. 原因：对互斥资源分配不当；进程推进顺序不当。
1. 死锁的四个必要条件：
   1. 互斥条件: 多个线/线程不能同时使用同一资源
   2. 请求与保持条件: 线程A在等待其他线程所持有的资源时不会释放自己已经持有的资源
   3. 非剥夺条件: 在自己使用完资源之前不能被其他线程获取
   4. 循环等待条件: 存在一种进程资源的循环等待链，链中每个进程已获得的资源同时被链中下一个进程所请求。
2. 死锁排查：`pstack: 查看指定进程的栈跟踪信息(函数调用过程)` + `gdb`
3. 处理产生死锁的办法有哪些？
   1. 预防死锁（破坏产生死锁的必要条件，最常见的是采用**资源按序分配法(大家保持同一顺序来申请资源)**来破坏循环等待条件）
   2. 避免死锁（银行家算法）；
   3. 检测死锁（资源分配图）；
   4. 解除死锁（剥夺资源 or 撤销进程）.
...
1. 互斥锁：加锁失败后，线程会释放cpu进行**线程切换**。
   1. 线程释放cpu，即线程被阻塞（由os内核操控从用户态陷入内核态），锁释放后os内核会在合适的时机唤醒线程。但这其中开销不小：**两次线程上下文切换的成本**：运行->睡眠->就绪
...
2. 自旋锁：加锁失败后，线程会**忙等待**，直到拿到锁
   ...
   3. 自旋锁比较简单，一直自旋，利用CPU周期，直到锁可用。需要注意，在单核CPU上，需要**抢占式的调度器**（即不断通过时钟中断一个线程，运行其他线程）。否则，自旋锁在单CPU上无法使用，因为一个自旋的线程永远不会放弃CPU。
...
1. **悲观锁**：假定多线程同时修改共享资源的概率比较高，很容易出现冲突，所以访问共享资源前，先要上锁
   1. 包括互斥锁、自旋锁、读写锁
2. **乐观锁**：假定冲突的概率低：先修改完共享资源，再验证这段时间内有没有发生冲突，如果没有其他线程在修改资源，那么操作完成，如果发现有其他线程已经修改过这个资源，就放弃本次操作
...
Linux为每个文件分配两个数据结构：
1.索引节点inode(index node): 记录文件元信息(描述数据属性的信息)，比如inode编号、**数据在磁盘的位置**、文件大小、权限、创建\修改时间等。**索引节点是文件的唯一标识**。位于磁盘的索引节点表中。
> 查看命令`stat`, `ls-l`
2.目录项dentry(directory entry): 记录文件名、**索引节点指针**、与其他目录项的层级关系。目录项是有内核维护的一个数据结构，缓存在**内存**中而非磁盘。

一个文件可以有多个目录项，但只有一个索引节点。即dentry和inode是多对一的关系。**硬链接的实现就是多个目录项中的索引节点指向同一个文件**。

**目录**也是文件，也用索引节点唯一标识，目录文件中存着子目录。存储在磁盘中。和目录项（缓存在内存）不是一个东西。为了避免频繁读取磁盘，os会把读过的目录用目录项缓存在内存中。
...
1.**磁盘的文件系统**：直接把数据存储在磁盘中，比如Ext 2/3/4、XFS等
> `df -lh` (disk free) 查看磁盘挂载
> `fdisk -l` 查看磁盘物理信息
...
> 挂载mount试讲一个文件系统连接到文件系统目录树的过程。使得文件系统中的文件和目录可以被用户访问和操作
---
使用文件
...
打开文件表：维护进程打开文件的状态和信息：
1.文件指针：**上次读写位置**
2.文件打开计数器：？？TODO 怎么还能让进程共享
3.文件磁盘位置：因为可能修改文件数据
...
7. 磁盘调度算法：
   1. 见第二章
   <!-- 2. FCFS：公平
   3. 最短寻找时间优先算法（Shortest Seek Time First,SSTF）：眼前最优未必总体最优，“饥饿”现象
   4. SCAN算法/电梯算法：磁头移动到最外侧磁道才往内移动。利于端头一侧。
   5. CSCAN：规定磁头单向移动，使个位置磁道的响应频率平均。 -->
...
==非阻塞I/O(Nonblocking)==：在数据为准备好的情况下立即放回，可以往下执行，此时应用程序不断轮询内核，直到数据准备好&&内核就爱那个数据拷贝到应用缓冲区，read调用才可拿到结果。
...
因此，**==无论read和send是阻塞I/O，还是非阻塞I/O都是同步I/O(Synchronous)调用==**。因为在 read 调用时，内核将**数据从内核空间拷⻉到用户空间的过程都是需要等待**的，也就是说这个过程是同步的，如果内核实现的拷⻉效率不高，read 调用就会在这个同步过程中等待比较⻓的时间。
...
服务端：socket() -> bind() -> listen()此时阻塞等待客户端连接 -> accept() (从全连接队列拿一个) -> read()阻塞等待客户端发送数据->clpse()
...
2. 多线程模型：每个客户端分配一个线程，pthread_create()创建线程，然后把已连接socket的fd交给线程函数即可；
   ...
   3. 参考图片[链接](https://mmbiz.qpic.cn/mmbiz_gif/GLeh42uInXTyY80RSpUTLjIMiaGGicv9zA55fIbicSuYiad7vYdyLD0usibPibYiaAjBDR0gQPzArnzYlWXOZRyQzub3Q/640?wx_fmt=gif&tp=webp&wxfrom=5&wx_lazy=1)
...
**我们知道单进程/线程会在read处阻塞（所谓的阻塞IO），多线程模型看起来不错，但这还不是真正的非阻塞IO，仅仅是使用多线程的手段使得主线程没有卡在read处不往下走罢了（这是用户层的小把戏），即此时的read依然是阻塞的。而且线程上下文切换也挺难顶的（虽然可以使用线程池数组解决一下，但依然是用户层的把戏）。
所以我们需要os提供点内核层的把戏。**
...
> IO复用的基本思想是事件驱动，服务器同时和多个客户端IO连接，当这个IO有可读或者可写事件发生时，服务器进行响应。
