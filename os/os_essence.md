...
- [操作系统笔记](#操作系统笔记)
  ...
  - [六、网络IO](#六网络io)
    - [零拷贝](#零拷贝)
    - [IO模型: 阻塞/非阻塞IO 同步/异步IO](#io模型-阻塞非阻塞io-同步异步io)
    - [服务器处理并发请求有哪些方式](#服务器处理并发请求有哪些方式)
    - [I/O多路复用](#io多路复用)
      - [select/poll](#selectpoll)
      - [epoll](#epoll)
      - [epoll工作模式 ET vs. LT](#epoll工作模式-et-vs-lt)
    - [高性能网络模式](#高性能网络模式)
      - [Reactor](#reactor)
      - [Proactor](#proactor)
...
* **==索引节点==** (inode): 存储文件元数据的数据结构，每个文件/目录有一个**唯一的inode**，包含了文件的信息：inode，大小，所有者，权限，时间戳，**数据块位置**等
  * `stat file`
...
1. 硬链接（基于索引节点的共享方式）：本质：**多个目录项dentry指向同一个索引节点(inode编号想通)**。
   1. 当使用rm删除文件时(源文件或硬链接文件)，只会删除对应的**目录项**，并减少inode链接计数器，只有当计数器为0，文件的inode和数据块才会被释放/彻底删除
   2. 优点：实现了异名共享。
...
1. 软连接（利用符号链实现文件共享）：本质：**新建一个文件**（拥有独立的inode），**文件内容(数据块)是另一个文件的路径**，所以访问该文件相当于访问了另一文件。**只有文件拥有者拥有指向索引节点的指针**。
...
### IO模型: 阻塞/非阻塞IO 同步/异步IO

**同步IO**：进程遇到io操作时（比如从磁盘读写文件，发送网络数据）会等待IO完成才进行下一步工作，即当前进程被阻塞。
**异步IO**：当需要执行IO操作时，只发出IO指令，不等待IO结果，非阻塞。


==**阻塞I/O**==：比如线程在read时阻塞，等待**内核准备好数据** && **把数据从内核缓冲区拷贝到应用缓冲区** (俩过程)，read才成功返回。
![picture 2](../images/045b9074ac323953f24c5e7ec4efb2cfaf61b135d038e578ae4bff4191175ea4.png){width=70%}
==**非阻塞I/O**==：在数据未准备好的情况下立即返回，可以继续往下执行，此时应用程序**不断轮询内核**，直到**数据准备好 && 内核缓冲区数据拷贝到应用缓冲区**，read调用才可拿到结果。(注意从内核缓冲区数据拷贝到应用缓冲区是同步的)
![picture 4](../images/ce03cd06b0984ae8ac302249e5526c5821ffff93c780379ae52814c5c545a8ba.png){width=70%}
...
![picture 5](../images/196aef96e529df80c3635024bcc47fdaba5ae0fb0c24d77d385e5ebb44b95cd6.png){width=70%}

**⽆论是阻塞I/O、还是⾮阻塞I/O、非阻塞I/O多路复用**，都是**同步调⽤**。等待还是有点耗时的。

==**异步I/O**(Asynchronous)==: **内核数据准备好 && 数据从内核态拷贝到用户态**，这俩过程都不用等待。内核缓冲区的数据会**自动拷贝**到用户缓冲区，应用程序无需主动发起read拷贝动作。
![picture 6](../images/460853b8309ff1627d1079ef4e1ec5485c3bfc1af263a9fd39ad49b2d357ddcc.png){width=70%}
...
![picture 15](../images/aad232ffc8d1e7344f00dc22053e2d7d13febd2dac3b729b2dc380f94eed8adc.png){width=60%}

> 故阻塞和非阻塞都是同步，故同步未必阻塞(可能非阻塞)，阻塞一定同步
...
> 当创建一个socket时，os会返回一个fd，该fd可用于后续的连接、发送和接受数据操作。每个fd对应一个读缓冲区read buffer和一个写缓冲区write buffer
...
* ==**select实现机制**==: **文件描述符集合fd_set**是长度为1024的数组，将想要检测的fd的fd_set值置为1，然后调用select()传入fd_set地址，内核会**拷贝**一份，然后去**遍历**该列表检测是否有读/写/异常事件发生(即检测read/write buffer是否就绪)，然后将结果写入到fd_set标记为可读/可写，我们将该fd_set**拷贝**会用户态，然后**遍历**拿出值为1的fd就可以执行具体的操作（如去read buffer读数据）    

* **Poll实现机制**：使用**动态数组**存储fd，突破了select的fd 1024个数限制；不像select传1000个fd，poll只传待检测的fd，但依然是线性结构；其他基本区别，缺陷也都还在

> select的fd_set分为：fd_set(读集合readfds,写集合writefds,异常集合exceptfds)

select/poll的缺陷：

* **拷贝开销大**：select和poll会将fd_set从用户态拷贝到内核态，然后内核修改完毕之后再拷贝回用户态
* **遍历开销大**：内核在检测集合时是**线性遍历**的，内核改完传给用户去，用户也需要线性遍历。两次遍历开销很大
* select可检测的fd上限是**1024**，在内核中写死了的，除非你重新编译内核进行修改...poll没上限(上限是硬件能力)。但poll只能在linux上使用，select可以跨平台

> 如果fd数量并不是很高，大多数fd都是激活的，选择使用select或poll，若并发量很高，采用epoll，但epoll无法跨平台。

> select可用于windows和linux，poll和epoll仅可用于linux

#### epoll

==**Epoll实现机制**==：

* epoll使⽤**红⿊树**来跟踪待检测的fd (我们调用epoll_ctl()把待检测fd加到红⿊树中)，红⿊树增删查是O(logn)，相比于线性结构查询效率更高
* epoll中的**内核和用户区使用共享内存**(基于mmap内存映射实现)，相比select减少了数据拷贝操作的开销
* epoll使用**事件驱动机制**，内核里维护了一个**链表来记录就绪事件**，只将有事件发生的fd集合传给用户，用户就不必遍历结果了，提高了检测效率
* epoll没fd上限，取决于系统硬件，和poll一样；select上限1024个fd。
  * epoll是解决C10K问题的利器，通过两个方面解决了select/poll的问题。select和poll难以应对

![picture 7](../images/ddf8b5832125aa630244ed98518e67c7b913a0fc71ec784a3466d8d2cdec424d.png){width=70%}

<!-- ![picture 6](../images/5d0335ad1942c8e3bc31c1f745cd02e0f7008c33e0329317aaa0a3996f94e5ac.png){width=70%} -->

#### epoll工作模式 ET vs. LT

ET(edge triggered) 边缘触发模式
LT(level triggered) 水平触发模式(默认)

* 使用边缘触发模式ET时，当被监控的Socket描述符上有可读事件发生时，服务器端只会从epoll_wait中**苏醒一次**，即使进程没有调用read函数从内核读取数据，也依然只苏醒一次，因此我们**程序要保证一次性将内核缓冲区的数据读取完**（高效，假设用户已经知道了
  * 一般搭配非阻塞IO
* 使用水平触发模式LT时，当被监控的Socket上有可读事件发生时，服务器端**不断地从epoll_wait中苏醒**，直到内核缓冲区数据被read函数读完才结束，目的是告诉我们有数据需要读取（用户友好，效率低
