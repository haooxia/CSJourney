...
- [操作系统笔记](#操作系统笔记)
  ...
  - [二、进程管理](#二进程管理)
    ...
    - [死锁](#死锁)
  - [三、内存管理](#三内存管理)
    - [（一）基础：将多个进程保存到内存中](#一基础将多个进程保存到内存中)
    - [（二）虚拟内存管理](#二虚拟内存管理)
  - [四、文件管理](#四文件管理)
    - [目录结构：索引节点 目录项 目录](#目录结构索引节点-目录项-目录)
    - [虚拟文件系统](#虚拟文件系统)
    - [文件的使用](#文件的使用)
    - [文件的存储/物理结构(非空闲磁盘块的管理)](#文件的存储物理结构非空闲磁盘块的管理)
    - [hard link \& symbolic link](#hard-link--symbolic-link)
    - [文件I/O (见网络部分)](#文件io-见网络部分)
  - [五、IO系统](#五io系统)
  - [六、网络系统](#六网络系统)
    - [Linux收发流程](#linux收发流程)
...
2. Linux内核设计理念：
   1. MultiTasK
      1. 并发concurrency：单核CPU让每个任务执行一小段时间，时间到就切换另外一个任务，从宏观⻆度看，一段时间内执行了多个任务。**并发会产生一种在并行的错觉**
...
3. windows内核
   1. multiTask
   2. **混合型内核**
   3. **PE 可移植执行文件**(portable executable)：扩展名.exe, .dll, .sys；所以windows和linux的可执行文件不能互相运行
...
1. **管道**pipeline：linux中的|，将前一个命令的输出作为后一个命令的输入，**单向**（半双工），如果需要相互通信(全双工)需要创建两个管道；
   ...
   4. 还有一种**命名管道(FIFO)**，`makefifo mypipe` ->`echo "helle" > mypipe` -> `cat mypipe`
...
3. **共享内存**：为了避免消息队列的频繁拷贝与系统调用开销，拿出一段虚拟地址空间，将其映射到**相同**的物理地址(将同一段物理内存连接到不同进程各自的地址空间中)。(==question: 共享内存是内核区域还是用户区域==)
   ...
   5. ![picture 21](../images/dbf56610c3c2fe0adb2a66ac713038f1c100480120517fca1c2d237898eaf53b.png)  
...
![picture 0](../images/d83ab6391ff8ba9785ab5c9f7cc8da76136a3db918406892832f17b0cbca4d61.png){width=60%}
...
3. **DMA**(Direct Memory Access)方式。DMA方式基本思想是，在**主存和IO设备**之间直接开辟数据通路，彻底解放CPU，使得设备可以不依靠cpu的情况下自行把输入放到主存。其特点是基本单位是数据块，所传送的数据是从设备直接送入内存的。仅仅在一个或多个数据块传输开始或结束时才需要CPU的干预，这个数据块的传输是在DMA控制器的控制下完成的。
   1. ![picture 24](../images/de9993d5649877d57f6621972117eb284d827584f7c66e31798530c298b3304d.png)
