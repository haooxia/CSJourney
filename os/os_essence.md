...
1. **管道**pipeline：`linux中的|`，将前一个命令的输出作为后一个命令的输入，是**单向**的（半双工），如果需要相互通信(全双工)需要创建两个管道；
   1. 匿名管道**只能用于父子进程**之间的通信。因为匿名管道没有实体文件，只能通过fork来复制父进程的文件描述符fd
   2. > **管道就是==内核==里面的一串缓存**。从管道的一端写入的数据，实际上是缓存在内核中的，另一端读取，也就是从内核中读取这段数据。
2. **命名管道(FIFO)**: **命名管道可以在不相关的进程建通信**，因为他有fd可以随意使用；
   1. `makefifo mypipe` ->`echo "hello" > mypipe` -> `cat mypipe`
3. **消息队列**：用于解决管道通信**效率低**和数据**格式受限**的问题；消息队列是**保存在内核中的链表中**(*true*)；
   1. 缺陷：进程写入数据到内核的消息队列，即将**数据从用户态拷贝到内核态**，开销较大，不适合**大数据**的传输 
   2. 这里的linux消息队列是用于**本地进程间通信**，不同于rabbitmq这种消息队列奥，rabbitmq这种是**分布式应用**间的消息传递
4. **共享内存**：为了避免消息队列的频繁拷贝开销，**允许不同进程将同一段物理内存映射/连接到它们各自的内存地址空间(逻辑地址空间)**，这些进程可以直接访问共享内存而无需借助内核。
   1. 优点：**速度最快**，因为**只需要在建立共享内存区域时需要系统调用**，一旦建立共享内存，所有的访问都可作为常规内存访问，无需借助内核。(eg `shmget`, `mmap`)
   2. 问题：多个进程**同时**修改同一共享内存可能引起**冲突**，所以保护机制：**信号量**上场
   3. ![picture 0](../images/9cd150ab3e522183a7965cf76c8ad9a9ba4b3eb934c7fc0744424037721e6900.png){width=80%}
6. **信号量**：实现**进程间**的互斥与同步，本质上是一个==整形计数器==。（通过PV操作保证多个进程按一定顺序访问共享资源）
7. **信号**：用于通知进程某个事件的发生，类似于处理器收到中断请求。`eg SIGINT, SIGKILL, SIGHUB`
   1. 信号是进程通信机制中**唯一的异步通信机制**，它可以在任何时候发送信号给某个进程，以迫使进程执行信号处理程序。用户、内核和进程都能生成和发送信号。
8. 基于网络的方式实现进程间通信
   1. socket -> HTTP：跨网络与不同主机上的进程之间通信
   2. rabbitmq消息队列
...
Q: **利用信号量实现临界区的互斥访问**?(简单且妙)

A: 对共享变量设置初始值为`1`的信号量`s`，表示该临界资源未被占用。然后在申请该资源前进行`P(s)`，之后进行`V(s)`操作进行释放。
...
Q: **利用信号量实现事件同步**?
A: 设置初值为`0`的信号量`s`，然后申请事件的时候P(s)，事件准备好的时候会V(s);
...
> 好好看图：数据从磁盘/socket读取到内核缓冲区(read系统调用)，再拷贝到

* 注意用户空间和内核空间分别有一个**用户缓冲区**和**内核缓冲区**
* 用户缓冲区主要用于减少用户进程与内核之间的数据传输次数，即 **==减少系统调用次数==**，用于**暂存数据**，等缓冲区满之后再通过**一次**系统调用**批量处理**
* 内核缓冲区主要是一个 **==缓存==** 作用，缓存从磁盘或其他设备(eg socket)读到的数据，后续再读可以命中该缓冲区，减少对物理设备的访问次数
  * 类似于mysql的bufferpool，类似于cpu和memory中的cache

### 零拷贝

> 系统调用：用户程序向操作系统内核**请求** 需要更高权限才能运行的**服务**，先从**用户态切换到内核态**，执行相应的内核代码，然后再从**内核态切换回用户态**（期间涉及到上下文的保存与恢复）
> 所以：**一次系统调用对应着两次上下文切换**

传统IO的方式，**从硬盘读取数据，然后再通过网卡向外发送**，我们需要进行**4次上下文切换(2次系统调用read+write)，和4次数据拷贝**，其中2次DMA拷贝(磁盘缓冲区->内核缓冲区，socket内核缓冲区->网卡缓冲区)，两次CPU拷贝
...
* **阻塞IO**: 进程发起IO操作后会被阻塞，直到操作完成才返回结果。比如read系统调用需要等待内核准备好数据+数据从内核缓冲区拷贝到用户缓冲区...适合对实时性要就不高的场景
* **非阻塞IO**: 应用发起IO后立即返回，不会阻塞，但**需要不断轮询检查**IO操作是否完成；一直占用cpu，鸡肋，所以纯非阻塞IO不常用
  * **非阻塞IO常常结合IO多路复用**：使用select/epoll等系统调用检测IO操作是否完成
    * 改进：原本轮询检查一个IO事件，现在阻塞检查多个fd
* **IO多路复用**: 通过select/poll/epoll系统调用，应用程序可以同时等待多个IO操作，其中任意一个IO操作准备就绪时，就去通知应用程序。适合需要同时处理多个IO操作的场景，比如**高并发的服务端程序**，需要同时处理多个socket连接的服务器程序
  * > 备注：调用select/epoll之后该线程本身将**阻塞**(注意：这里阻塞是等待多个fd的状态变化，即等待多个IO操作，不同于阻塞IO的等待单个IO操作)，等待内核检测到fd有事件发生时，**自己这个线程本身去处理该事件**；
  * > 即如果不用IO多路复用，对于多个IO事件，你就得为每个IO事件开一个线程去阻塞地等待，或者不断轮询，ie，IO多路复用就是让一个线程检测多个IO事件
  * ==**redis**== 它也是这样：主线程调用epoll的`epoll_ctl`将多个客户端fd注册到红黑树，然后阻塞等待`epoll_wait`让内核去检测IO事件，一旦有事件发生，**主线程**被唤醒，处理事件，处理完毕**再阻塞**，再监听下一个事件。
    * **用户空间**的redis主线程阻塞后，交给**内核**去检测fd，检测到后唤醒用户空间的redis主线程
