...
一言以蔽之：**IO多路复用就是一个进程/线程维护多个Socket，这个多路复用就是多个socket连接复用一个进程/线程**；避免创建过多线程导致的上下文切换的开销。

Linux提供了三种IO多路复用的系统调用：select, poll, epoll，**进程可以通过一个系统调用函数从内核中获取多个事件**。

* select实现机制: 将已连接的Socket都放到⼀个 **⽂件描述符集合fd_set**，然后调⽤select函数**将fd_set集合拷⻉到内核⾥**，让内核来检查是否有⽹络事件产⽣，检查的⽅式很粗暴，就是通过**遍历**fd_set的⽅式，当检查到有事件产⽣后，将此Socket**标记为可读或可写**，接着再把整个fd_set**拷⻉**回⽤户态⾥，然后⽤户态还需要再通过**遍历**的⽅法找到可读或可写的Socket，再对其处理。
  * 缺点：
    * 每次调用select，都要把fd_set从用户态拷贝到内核态，没必要全都拷贝吧
    * 每次select内核都要**遍历**fd_set，开销大
    * 一般fd_set大小限制为1024
* Poll实现机制：使用动态数组存储fd，突破了select的fd 1024个数限制，其他并没啥区别，缺陷也都还在; 他俩**都用线性结构存储socket集合，因此都需要遍历fd_set找到可读可写的socket，O(n)复杂度，也都需要在用户态和内核态间拷贝fd_set**
* Epoll实现机制：
  * epoll在内核⾥使⽤**红⿊树**来跟踪进程所有待检测的fd/socket，**把需要监控的socket通过epoll_ctl()函数加⼊内核中的红⿊树⾥**，红⿊树增删查⼀般是O(logn)，通过对这棵树的管理，不用像select/poll那样每次传入整个socket集合给内核，epoll每次只需要传⼊⼀个待检测的socket，减少了内核和⽤户空间⼤量的数据拷⻉和内存分配。
  * 第二，epoll使用**事件驱动**的机制，内核里维护了一个**链表来记录就绪事件**，只将有时间发生的socket集合传给应用程序，不需要像select/poll那样轮询扫描整个socket集合(包含有和无事件的socket)，大大提高了检测的效率。
