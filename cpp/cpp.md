...
42. **C/C++语言的内存布局**
    ...
    | ----------- | ------------------------------------------------------------ |
    | 全局/静态区 | 存放**全局变量**、**静态变量**(static)、**常量和const修饰的全局变量）**，该区域的数据在程序结束后由os释放 |
    | 栈区        | 由编译器自动分配释放，存放函数的参数值和返回值、局部变量等   |
    | 堆区        | 由程序员分配释放，若程序员不释放，程序结束时由os回收         |

43. **堆heap和栈stack的区别：（见c++笔记）**

    堆中的数据需主动malloc和free（动态内存分配），而栈由系统默认申请和释放（静态内存分配）。
