...
Java程序员把内存控制权力交给JVM，而非像c++一样需要程序员手动new/delete，不容易出现内存泄漏和内存溢出问题

总结：

我们从线程共享和线程私有两个角度来分析Java内存区域(jdk1.7):

**线程共享**：

1. **堆**heap：所有对象实例和数组都在堆分配内存，垃圾回收器GC会管理其中的对象。堆中还包含了**字符串常量池**String Constant Pool，用于存储**字符串字面量**。
2. **方法区**method area: 存储类信息、字段信息、方法信息、运行时常量池等。其中**运行时常量池**Runtime Constant Pool是方法区的一部分，用于存储编译器生成的各种字面量(字符串、数值等)和符号引用(方法名、类名等)。

**线程私有**：

1. **虚拟机栈**VM stack: 每个线程启动时都会创建一个虚拟机栈(存储多个栈帧)，每个方法调用都会创建一个栈帧，包括局部变量、**操作数栈**、方法返回地址等，方法执行结束后自动出栈。（无需GC回收）
2. **本地方法栈**Native Method Stack: 专门存储native method的调用信息，与虚拟机栈类似。
   1.  native method用c/cpp实现的方法，通常借助系统调用去内核态操控系统资源
3. **程序计数器**PC register: 记录当前线程下一条要执行的字节码指令的地址。方便线程切换后恢复执行位置。

**本地内存**Direct Memory：用于高性能数据传输，不属于JVM内存区域。可以通过NIO等方式直接分配内存。属于OS内存。

> 1.8将方法区替换为元空间，且使用本地内存。
> 1.7的运行时常量池属于方法区/永久代，1.8属于元空间。（但其实方法区/永久代/元空间基本算同一玩意儿吧
...
> 本地内存在JVM内存结构之外
> 字符串常量池(ie 串池)：专门存储string字面量的(为了重用)，在堆中（1.7之前在方法区，1.7之后移到堆）
...
1. **方法区/元空间/永久代**：方法区是一个逻辑概念，元空间和永久代都是对JVM规范中**方法区的实现**。
   1. 方法区存储被JVM加载的：
      1. **类信息**(类名, 修饰符, 接口, 父类...)
      2. **字段信息**(类中所有字段的名称, 类型, 修饰符)
      3. **方法信息**(名称, 参数类型, 返回类型, 方法的字节码)
      4. **运行时常量池**: 包含编译器生成的字面量和符号引用
      5. ==**static静态变量**==
      6. 即时编译器**JIT编译后的代码缓存**
...
1. **新生代 (Young Generation)**
   ...
   3. > 新生代的垃圾收集主要采用 **==标记-复制算法==**，每次分配内存只使用Eden和一个survivor区，垃圾收集时，将eden和survivor中还存活的对象一次性复制到另一块survivor上，然后清理掉eden和另一块survivor
...
对象一般先在新生代中分配，然后某些满足条件的对象会进入到老年代中。

1. **长期存活的对象**: 对象在年轻代中存活年龄超过15后（每次GC，没被回收的对象年龄++）。每次GC后，没被回收的对象的年龄++；通过`-XX:MaxTenuringThreshold`修改。
2. **大对象直接分配到老年代**：为了避免在新生代频繁复制大对象(因为新生代时标记-复制嘛)，jvm会将大对象直接在老年代中分配（所谓"大"也是jvm参数控制的阈值）
3. **survivor空间(To空间)不足**：新生代进行minor GC时，如果发现存活的对象在另一个survivor区存不下，那这些对象就直接存到老年代
...
* **Minor GC** (Young GC)
  ...
  * **触发条件**：当Eden区满时，JVM会触发一次Minor GC，将Eden区和一个Survivor区中的**存活对象移动到另一个Survivor区==或老年代==**（**只有当对象的年龄达到阈值(default=15)时，这些对象才会被晋升到老年代**
...
* 如果使用了**ThreadLocal**而没有及时调用remove()，在使用线程池时可能会导致内存泄漏，因为线程会持有对某些对象的引用。（不太懂原理）
...
* **引用计数法（Reference Counting）**
  ...
  * 缺点：无法处理循环引用的问题（即两个对象相互引用，但不再被外部使用）
* **可达性分析算法（Reachability Analysis）**(主要)
  * 原理：从**一组**称为**GC Roots**的对象出发，向下追溯它们引用的对象，以及这些对象引用的其他对象，以此类推。如果一个对象无法从GC roots到达，则可以被回收。
    * **GC Roots对象包括**：虚拟机栈中引用的对象（方法的局部变量、参数），本地方法栈中引用的对象（native方法中的引用），方法区中静态变量引用的对象（存在方法区的类变量），运行时常量池中引用的对象（eg 字符串常量）等

![picture 2](../images/6c048f048c958b317a2c316b2911a8509d7b78594a83039953b26403d00e3a21.png)  

### 垃圾回收/收集算法 ☆☆

垃圾回收Garbage Collection, GC是JVM中自动管理内存的重要机制，通过一系列算法识别和回收不再使用的对象，来**释放堆内存**。常见有四种：

1. **标记-清除算法 (Mark-and-Sweep)**：最**基础**的GC算法，分为**两阶段**：一是**标记阶段**，根据可达性分析**标记**出所有可达的对象，二是**清除阶段**，回收所有不可达/未标记的对象
   1. 问题：清除后**内存碎片化**，即产生大量不连续的碎片，导致大对象无法分配内存。
   3. 问题：效率低，需要两次遍历堆内存，耗时。
2. **复制算法 (Copying)**: 将内存分为两块(From和To)，每次只用其中一块，解决了内存碎片化问题。也分两阶段：一是**复制阶段**，当一块内存用完，将存活的对象复制到另一块内存，按序排列。二是**清理阶段**，清空原来的内存块。
   1. 优点：高效、且没有内存碎片化问题
   2. 缺点：内存利用率低，需要双倍的内存空间
3. **标记-整理算法 (Mark-and-Compact)**：是标记清除算法的改进。标记之后不会直接清理。而是**将所有存活对象都移动到内存的一端**，然后清理整理后的边界以外的内存，解决内从碎片化问题。
   2. 适合老年代old generation，因为老年代中对象的存活率较高，复制成本较大
4. **分代回收算法(Generational Collection)**：**主流**的GC算法。根据对象存活周期(经历过的GC次数)，将堆内存划分成了**新生代和老年代**。
   1. 对于新生代，使用**复制算法**，因为新生代中的对象大多数是**短命**的。（新生代进一步划分为Eden区和两个Survivor区(From 和 To)）；
   2. 对于老年代，使用**标记-整理算法**，因为老年代中的对象大多数是长命的。
   3. 缝合怪，整合了不同算法的优点。
   4. 对象创建时，一般在新生代申请内存，当经历一次GC之后如果对还存活，那么对象的年龄+1。当年龄超过阈值后，如果对象还存活，那么该对象会进入老年代。阈值默认是15

### 垃圾回收/收集器 ☆☆

TODO

GC算法是方法论/理论策略，GC器是具体实现。最常用的是CMS和G1收集器。

1. GMS收集器(Concurrent Mark Sweep)：是以**最小化停顿时间**为目标的垃圾收集器，适用于需要高响应的应用场景（如Web应用）。其基于“**标记-清除算法**”，回收流程包括以下阶段：
   1. 停止所有用户线程，启用一个GC线程进行初始标记（Stop The World），标记 GC Roots能直接引用的对象，停顿时间短
   2. 由用户线程和GC线程并发执行，进行**并发标记**，用户线程和GC线程并发执行，完成从GC Roots开始的对象引用分析
   3. 启动多个GC线程进行重新标记（Stop The World），修正并发标记期间用户线程对对象引用的变动，停顿时间稍长但可控
   4. 启动多个用户线程和一个GC线程，进行并发清除，清理不可达对象，清理完成后把GC线程进行重置
