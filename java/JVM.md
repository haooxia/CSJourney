...
1. **强引用 Strong Reference**: 最常见的，eg `Person a = new Person()`，任何对象只要被强引用关联，GC**绝不回收**他们（即使内存不足），如果想要回收对象，必须显式地将强引用置为null
2. **软引用 Soft Reference**: 描述一些非必需但仍有用的对象(可有可无)。当内存不足时，GC会回收软引用对象(如果内存足够不会回收)
...
4. **虚引用PhantomReference**: 最弱，形同虚设，主要用来**跟踪对象被垃圾回收的活动**。
