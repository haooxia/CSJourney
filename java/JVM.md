...
> 本地内存在JVM内存结构之外
> 字符串常量池(堆中)只存储字符串常量，运行时常量池(方法区:堆/本地内存)存储编译期生成的各种字面量和符号引用

apparently, 线程共享堆、方法区和*直接内存*；线程私有程序计数器、虚拟机栈和本地方法栈

1. **方法区/元空间/永久代**：元空间和永久代都是对JVM规范中**方法区的实现**。
   1. 方法区存储：**类信息**(类名, 修饰符, 接口, 父类...), **字段信息**(类中所有字段/变量(包括static变量)的名称, 类型, 修饰符), **方法信息**(名称, 参数类型, 返回类型, 方法的字节码), **运行时常量池**, 即时编译器**JIT编译后的代码缓存**等数据
...
2. **虚拟机栈**：每个线程有一个私有的栈，随着线程的创建而创建。栈由一个个 **栈帧(stack frame)** 组成，每个java方法会创建一个栈帧，调用结束后弹出该帧。栈的大小可以固定也可以动态扩展。
   1. 栈帧中存放了局部变量表（参数+局部变量：基本类型+对象引用(非对象本身)）、操作数栈(计算过程的中间结果)、方法返回地址等信息
...
3. **本地方法栈**：与虚拟机栈类似，native方法通过本地方法栈实现，同样创建一个栈帧
   1. 由于java有时不能直接操作os底层，所以有时依赖c/cpp
4. **程序计数器PC**：存储当前线程**下一条**要执行的字节码指令的**地址**。在任何一个确定的时刻，一个处理器内核只会执行一条线程中的指令。因此，为了线程切换后能恢复到正确的执行位置，每条线程都需要一个独立的PC(线程私有)。
...
6. **直接内存(direct memory)**：直接内存不属于JVM内存区域，属于OS内存。在JDK1.4中新加入了 **NIO(New Input/Output)** 类，引入了一种基于通道(Channel)与缓冲区（Buffer）的I/O 方式，它可以使用native函数库直接分配堆外内存，然后通过一个存储在Java堆中的DirectByteBuffer对象作为这块内存的引用进行操作。这样能在一些场景中显著提高性能，因为避免了在Java堆和Native堆中来回复制数据。
...
二者是方法区的实现，jdk7是永久代PermGen(可以理解为堆的一部分)，jdk8是元空间MetaSpace(本地内存)，之所以替换是为了解决一些问题：
...
2. **生命周期**：栈中的数据生命周期确定，当一个方法调用结束时，其对应的栈帧(内含的数据)就会被销毁。**堆中的对象生命周期不确定**，对象会在垃圾回收机制**GC检测到对象不再被引用时才被回收**
3. **存取速度**：**栈的存取速度更快**，因为栈遵循先进后出LIFO的原则，简单快速。对象在堆上的分配和回收需要更多的时间，而且垃圾回收机制的运行也会影响性能
4. **存储空间**：栈的空间更小，且固定，由os管理。当栈溢出时，通常是因为**递归过深或局部变量过大**。堆的空间较大，**动态扩展**，由JVM管理。堆溢出通常是由于**创建了太多的大对象或未能及时回收不再使用的对象**
5. **可见性**：栈中数据是线程私有的，堆中数据是线程共享的
...
1. **新生代 (Young Generation)**
   1. **Eden`[ˈiːdn]` Space**: 大多数新创建的对象首先存放在这里。Eden区相对较小，当Eden区满时，会触发一次**Minor GC（新生代垃圾回收）**
   2. **Survivor Spaces**: 通常分为两个相等大小的区域，称为S0和S1。**每次Minor GC后，存活下来的对象会被移动到其中一个Survivor空间或者老年代**，以继续它们的生命周期。这两个区域轮流充当对象的中转站，帮助区分短暂存活的对象和长期存活的对象。
2. **老年代 (Old Generation/Tenured Generation)**: 存放过一次或多次Minor GC仍存活的对象会被移动到老年代。
   1. **老年代中的对象生命周期较长**，所以空间比新生代大。
   2. 老年代的垃圾回收叫**Major GC**，发生频率更低，但执行时间更长
3. **方法区**: jdk7使用**永久代**实现，存在堆中，但容易发生内存溢出。jdk8使用**元空间**取代，并移到了本地内存，解决了方法区内存溢出问题。用于存储类的元数据信息，如类的结构信息（如字段、方法信息等）
...
* **Minor GC** (Young GC)
  ...
  * **触发条件**：当Eden区空间不足时，JVM会触发一次Minor GC，将Eden区和一个Survivor区中的**存活对象移动到另一个Survivor区或老年代 (Old Generation)**
  * **特点**：**频繁**发生，因为年轻代中对象的生命周期较短，回收效率高，暂停时间相对较短
* **Major GC** (Old GC)
  * **作用范围**：**主要针对老年代**进行回收，但不一定只回收老年代
  * **触发条件**：当老年代空间不足时，**或检测到年轻代对象晋升到老年代的速度过快**，可能会触发Major GC
  * **特点**：相比Minor GC，Major GC发生的**频率较低**，但每次回收可能需要**更长时间**，因为老年代中的对象**存活率较高**，即需要回收的对象更少
* **Full GC**
  * **作用范围**：对**整个堆内存**（包括年轻代、老年代以及永久代/元空间）进行回收。
    * **Q: 永久代能不能算堆内存**：永久代和堆的老年代是连续的，所以可以被视为堆的一部分。元空间肯定不能算了
  * **触发条件**：
    * 直接调用`System.gc()`或`Runtime.getRuntime().gc()`时，虽然不能保证立即执行，但JVM会尝试执行Full GC
    * Minor GC时，如果存活的对象无法全部放入老年代，或者老年代空间不足以容纳存活的对象，则会触发Full GC，对整个堆内存进行回收
    * 当永久代或元空间空间不足时
...
1. **强引用StrongReference**: 代码中普遍存在的赋值方式，eg `Person a = new Person()`，**这类对象绝不会被GC回收**
2. **软引用SoftReference**: 可有可无，如果内存空间足够，GC不会回收它，在**发生内存溢出之前**，GC会回收这些对象。
   1. 软引用可用来实现内存敏感的高速缓存。
3. **弱引用WeakReference**: 强度比软引用更低一点，GC一旦发现弱引用的对象，不管当前内存够不够，都会回收。
   1. 不过，由于GC是一个优先级很低的线程，所以不一定能够很快发现具有弱引用的对象
4. **虚引用PhantomReference**: 又名**幻影引用**，最弱，形同虚设，主要用来**跟踪对象被垃圾回收的活动**。
   1. 必须和引用队列ReferenceQueue一起使用

> 弱引用了解吗?举例说明在哪里可以用? 暂略

### 内存泄漏 vs. 内存溢出

* **内存泄露**：程序中**不再需要的对象仍然被引用，导致这些对象无法被GC回收**，从而占用内存，**内存泄漏可能导致OOM**(占满了，jvm申请内存找不到空间了)。
* 内存泄露常见原因（慢慢体会吧）：
  * **static静态集合**：使用静态数据结构（如HashMap或ArrayList）存储对象，且未清理
  * 如果使用了**ThreadLocal**而没有及时调用remove()，在使用线程池时可能会导致内存泄漏，因为线程会持有对某些对象的引用。
  * **事件监听**：未取消对事件源的监听，导致对象持续被引用
* **内存溢出**：JVM在**申请内存时无法找到足够的内存**，最终引发OOM。这通常发生在堆内存不足以存放新创建的对象时。
* 内存溢出常见原因：
...
![picture 0](../images/3270d71c22b73db0ec61f298afd6c9f7536f9a91e9151c69ecbefb6558523b3e.png)  

1. **类加载检查**：虚拟机遇到一条new指令时，首先检查这个指令的参数是否能在**常量池**中定位到一个**类的符号引用**，并且检查这个**符号引用代表的类是否已被加载过、解析和初始化过**。如果没有，那必须先执行相应的 **==类加载过程==**
2. **分配内存**：虚拟机将为新对象分配堆内存。（对象所需的**内存大小在类加载完成后便可确定**）
3. **初始化零值**：将分配的内存空间都初始化为零值（不包括对象头），这一步操作**保证了对象的实例字段在Java代码中可以不赋初始值就直接使用**，程序能访问到这些字段的数据类型所对应的零值。
4. **设置对象头**：对象头包含了对象是哪个类的实例、对象的hashcode、对象的GC分代年龄等信息。还有偏向锁相关信息
