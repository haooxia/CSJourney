...
- [Java](#java)
    - [基础](#基础)
      ...
      - [java vs. c++](#java-vs-c)
      - [如何理解java只有值传递 pass-by-value](#如何理解java只有值传递-pass-by-value)
    - [keywords](#keywords)
      - [Access Modifier](#access-modifier)
      - [abstract final static](#abstract-final-static)
      - [final vs. finally vs. finalize()](#final-vs-finally-vs-finalize)
    - [数据类型](#数据类型)
      - [基本类型 vs. 包装类型 ☆ / int vs. Integer](#基本类型-vs-包装类型---int-vs-integer)
...
* 源代码 `test.java`: 人写的，计算机无法直接理解和执行
* 字节码 `test.class`: `javac`编译器编译后的**中间表示形式**，==**平台无关**==，可以实现“一次编写、到处运行” (write once, run anywhere)
  * C语言编译后会直接生成特定平台的机器码，可以直接在该平台运行。 `gcc -> a.out`
  <!-- * `javac test.java -> test.class` -->
  * `javap -c test.class`: 查看字节码（将class反编译为人类可读的格式，字节码本来是二进制指令）
* 机器码：字节码通过JVM或者JIT即时编译器转化为机器可以直接执行的机器码


> 现代JVM引入了**即时编译技术JIT**(Just-in-time compilation)：JIT会监控程序执行情况，识别出频繁执行的“热点代码”（**字节码**），然后将其编译为机器码，并**缓存在内存**中，以便后续直接执行，避免**重复的解释**过程。

---

java执行过程：

1. `javac`编译器将java源代码编译为字节码文件（.class）
2. JVM将.class字节码文件加载到内存中，由**类加载器**完成，类加载过程如下：
   1. 加载：类加载器从文件系统/网络中加载.class到内存中
   2. 验证：验证字节码的合法性和安全性，确保其符合JVM规范
   3. 准备：为类的static变量分配内存并设置**默认值**
   4. 解析：jvm将**运行时常量池中的符号引用**转换**直接引用**
   5. 初始化：jvm执行**类构造器方法**`<clinit>()`来初始化类，
      * 该方法会初始化类的静态变量和静态代码块（初始化不是设置默认值，是显式赋值
      * 注意：`<clinit>()`方法是由编译器自动生成的，并非用户写的构造函数
...
#### 如何理解java只有值传递 pass-by-value

* 当传递基本数据类型时,传递的是该值的**一个副本**。函数方法内对形参的修改不会影响到实际参数。
* 对于引用类型(对象or数组)，也是**值传递**，这个值是指**对象的引用的副本**（即对象的内存地址），而非**原始的对象的引用**(c++是)。==方法内部可以通过该副本访问和修改对象的属性（影响到原始对象），但不可以修改原始引用指向的对象（不可指向新对象）==
...
* protected: protected修饰的成员对**同一包内的其他类**和**不同包中的子类** 可见。（包内可见 & 不同包子类可见）
* default: 只能被**同一包内的其他类**访问（包内可见）
* private: private修饰的成员只能被该**类自己**的方法访问。（类内可见）
  * 子类可以继承，但不能访问
...
当存储一个基本类型到需要用到对象的场景中（eg集合），java编译器会自动装箱，将其转为包装类对象，底层是调用包装类的valueOf()。
...
内部缓存了个Interger对象数组吧，len=256

```java
public static Integer valueOf(int i) {
    if (i >= IntegerCache.low && i <= IntegerCache.high)
        return IntegerCache.cache[i + (-IntegerCache.low)];
    return new Integer(i); // 超出才会new
}
```
...
* **语法形式**：局部变量在方法、构造函数或代码块内声明，只在代码块中有效；成员变量声明在类中，但在方法、构造函数或代码块之外。
...
* **==存储方式==**：基本类型的局部变量存在栈中，引用类型的局部变量（引用本身还是在栈中，所以说局部变量在**栈**中没啥问题）；static成员变量属于类，存在**方法区**；non-static成员变量属于实例instance，跟着对象存在**堆**中。
  ...
  * 补充一点：`Student s = new Student(...);`，该对象在堆中，变量s在栈中，变量s存放的是该对象在堆中的内存地址。如果你要拿s和另一个s1做==比较，那么其实比较的是两个对象的内存地址是否相同，而非两个栈地址，因为~~s这个值就是堆中对象的地址~~（可以这么理解，但实际s的值是对象的引用，间接指向对象的内存地址，大概如此吧
* **生命周期**：局部变量随着块/方法的调用而生成和消亡；static成员变量与类的生命周期一致，在类加载时分配内存，在程序结束时释放内存；non-static成员变量和对象的生命周期一致，随对象创建而分配，回收而释放。
* **默认值**：如果不赋初值，局部变量不会自动赋值，即 **==局部变量必须初始化==**；而成员变量会自动为类型的默认值(e.g., 0, null, false)。
...
* ⾯向过程将系统视为**⼀系列的过程或函数**，强调算法和**流程**。更适合**简单**的、**线性**的任务。模块间**耦合严重**，但**效率高**
* ⾯向对象会先**抽象出具有状态、行为的对象**，然后⽤**对象执行方法**的⽅式解决问题，强调封装、继承和多态，更**容易扩展和维护**。修改⼀个对象不会影响到其他对象，适合处理**复杂**的系统。

#### 面向对象六大原则 / SOLID（终于懂点了

* 单一职责原则 (S-Single)：一个类应该只负责一项职责。
  * eg 考虑一个员工类，它应该只负责管理员工信息，而不应负责其他无关工作。
* 开闭原则 (O-Open): 类/模块/函数应该对扩展开放，对修改封闭。也就是说，应该能够在不修改现有代码的情况下增加新功能。
  * eg 通过制定接口来实现这一原则，比如定义一个图形类，然后让不同类型的图形(圆形、方形)继承这个类，而不需要修改图形类本身。
* 里氏替换原则 (L-liskov)：**子类对象必须能够替换掉父类对象**，而不影响程序的正确性。也就是说，子类必须**能实现**父类的功能。
...
* 实例化new对象时，系统自动调用ctor完成对象的初始化
* 如果未定义ctor，默认提供无参ctor(它内部啥也不干)；**如定义了ctor，系统不会提供无参ctor**，如需要，自行显式定义
...
class Person {
    ...
    int age;
    // 有参构造函数
    public Person(String name, int age) { // 无返回值 & public
...
* **封装Encapsulation**：将对象的属性和方法**包装**在一起，通过访问控制修饰符对外**隐藏**内部实现细节(eg 属性)，只**暴露**必要的接口/方法供外部访问和修改；
  * 一般就是私有化属性（防止外部直接访问）->提供public的setter和getter（来控制访问和修改对象的状态）
    * **为何不直接public属性呢？** 答：避免随意修改。比如age，我们可以再setter中添加限制条件，eg `if(age >= 0 && age <= 200)`才让修改，避免随意修改。
      * 也即增强代码的可维护性和安全性，防止外部代码对对象内部状态进行不合理的修改。（可以进行**数据验证**）
* **继承Inheritance (is-a)**：允许一个类继承另一个类的属性和方法，实现代码的重用。子类可以**扩展**父类的功能(加方法)或**重写**父类的方法
  * extends关键字: `class Dog extends Animal`
...
* **多态Polymorphic**：**同一个方法调用在不同对象上**表现出不同的行为。多态分为编译时多态（方法重载）和运行时多态（方法重写）。重写的底层原理是动态绑定
...
* 不同点：
  ...
  * 抽象类可以**包含抽象方法和非抽象方法**。而接口的方法一般来说都是**抽象的**，不能有方法体（jdk8之后允许有默认方法体）
  * 抽象类可以有构造函数，接口不可
...
* 对象拷贝：**创建一个新的对象**，分为shallow copy和deep copy
  * shallow copy: 如果原对象的成员变量是**基本数据类型**，复制一份给克隆对象；如果原对象的成员变量是**引用类型**，则只会复制该引用，两个引用指向同一对象，即**不会复制指引所指向的实际对象本身**。（修改新对象中的引用类型会影响到原对象）
    * Object类提供了`clone()`方法，默认是浅拷贝（需要重写，方法体写作: `return super.clone()`）
  * deep copy: 无论是值类型还是引用类型都会完完全全的拷贝一份，在内存中生成一个新的对象，得到一个**独立**的副本。即**会递归地复制对象内部的所有引用类型数据**
    * 通过**序列化**实现，或手动遍历**为每个引用类型属性创建新的实例**(重写clone())

> 深拷贝可以通过重写Object类下clone()来手动实现，浅拷贝也需要重写clone()，因为需要改变其可见性(默认clone方法是protected的，重写public后才可对外部可用)
...
* Object中默认的`equals()`就是`==`，但**很多类重写了该方法**，以判断俩对象的属性是否相同(eg, Integer, String)
  * **注意自己的类需要自己重写equals()**，需要的话
...
> 补充：这里的s1==s3是正确的，s1的值是常量池中的对象的地址（注意s1的值不是栈地址），s3同样，所以s1当然和s3相同。
> 这里说s1存的是地址其实不太严谨，其实存的是对象的引用，而引用可以间接指向内存地址（ok 很抽象了 先不谈就了
...
### 反射 ☆☆ ([TODO](https://www.nowcoder.com/issue/tutorial?zhuanlanId=MQ75nB&uuid=33103f122c174431bff5175ec7b6c634#48nwbb))
