...
- [Java](#java)
    ...
    - [OOP](#oop)
      ...
      - [面向对象六大原则 / SOLID（有亿点抽象](#面向对象六大原则--solid有亿点抽象)
...
* [IO流](https://javabetter.cn/sidebar/sanfene/javase.html#_42-java-%E4%B8%AD-io-%E6%B5%81%E5%88%86%E4%B8%BA%E5%87%A0%E7%A7%8D)
...
* protected: protected修饰的成员对**同一包内的类**和**不同包中的子类**可见
* default: 只能被**同一包内的类**访问
...
* 存储方式：基本类型的局部变量存在栈中，基本类型的static成员变量存在方法区，基本类型的non-static成员变量存在堆中(算对象嘛)。而**包装类型属于对象类型**，均存在堆中。（static修饰的包装类型，对象本身也是存在堆中的，只是引用变量存在方法区）
  * 注意基本类型未必都存在stack中
...
* **存储方式**：基本类型的局部变量存在栈中，引用类型的局部变量（引用本身还是在栈中，所以说局部变量在**栈**中没啥问题）；static成员变量属于类，存在**方法区**(待详解)；non-static成员变量属于实例instance，跟着对象存在**堆**中。
...
#### 面向对象六大原则 / SOLID（有亿点抽象

* 单一职责原则 (S-Single)：一个类应该只负责一项职责。
  * eg 考虑一个员工类，它应该只负责管理员工信息，而不应负责其他无关工作。
* 开闭原则 (O-Open): 软件实体（类、模块、函数等）应该对扩展开放，对修改封闭。
  * eg 通过制定接口来实现这一原则，比如定义一个图形类，然后让不同类型的图形继承这个类，而不需要修改图形类本身。
* 里氏替换原则 (L-liskov)：**子类型必须能够替换掉它们的父类型**。
  * 如果A是B的子类型，那么B类型的对象在没有知道它是S类型的情况下，能够一直使用S类型的对象。
* 接口隔离原则 (I-Interface)：一个类不应该依赖那些它不需要的接口，即接口应该小而专。例子：通过接口抽象层来实现底层和高层模块之间的解耦，比如使用依赖注入。
* 依赖倒置原则 (D-Dependency)：高层模块不应该依赖低层模块，二者都应该依赖抽象；抽象不应该依赖细节，细节应该依赖抽象
...
* 实现方式不同：抽象类使用extends，且只能继承一个；接口使用implements，且能够多继承
...
1. 父类static变量和static代码块
   1. 实例化对象之前，肯定是要先加载对象的，即static
2. 子类static变量和static代码块
   1. 因为子类可能依赖父类的静态成员
3. 父类普通变量和普通代码块
4. 父类构造方法
...
> hashcode认为不存在则一定不存在，hashcode认为存在则未必存在
...
**详述编译时类型检查：**
比如ArrayList支持泛型，所以支持编译时类型检查，而内置Array不支持。

所以ArrayList你放入了和指定类型不同的元素，在编译java源码时即会报错，而Array则会在运行时才会报错。
编译时异常的话，就意味着你开发者在编译阶段就可以进行排查，在代码交付之前就可以尽量减少错误，提升用户体验；而如果运行时报错，那生产环境如果因此而异常崩溃，那就麻烦了
