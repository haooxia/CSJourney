...
* 核心思想：**为每个数据行维护多个版本**。当数据被修改时，保存该数据的旧版本，并创建一个新的版本。这样，读操作就可以访问到某特定时间点的数据快照，而**无需等待写操作完成**。
...
1. **避免不必要的列/字段**：eg 少用`select *`，而是直接指定所需字段，以减少数据传输量
2. 进行**索引优化**：前缀索引、覆盖索引、主键自增、防止索引失效
3. 进行**Join优化**：
...
4. 进行**排序优化**：频繁排序的字段，可以提前创建好索引，这样查询时如果要求按该字段排序，自然读出来就是了；否则还需要对结果集进行排序
5. 进行**分页优化**：
