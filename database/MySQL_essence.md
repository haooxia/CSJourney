...
1. **捕获低效SQL**：MySQL中`long_query_time`**默认值是10秒**，即执行时间超过10秒的**SQL语句文本**会被记录到**慢查询日志**中(.log文件)
   ...
   <!-- 2. **方法二：**：通过`show profiles`命令查询当前会话中**所有的sql语句的执行耗时情况**，可以分析所有的语句，更强大；此外，通过`show profile for query queryId`命令可以查看指定sql的各个阶段的详细耗时时间（了解一下 -->
