...
- [MySQLf](#mysqlf)
  - [一、基础](#一基础)
    - [执行select语句，期间发生了什么](#执行select语句期间发生了什么)
      - [连接器](#连接器)
      - [查找缓存](#查找缓存)
      - [解析器解析SQL](#解析器解析sql)
      - [执行SQL](#执行sql)
...
* **一致性**（Consistency）：事务完成时，必须使所有的数据都保持一致状态。（？）包括数据和约束。
...
**读提交RC(Read Committed)**: 一个事务提交之后，该变更才能被其他事务看到
**可重复读RR(Repeatable Read) (mysql default)**: 一个事务执行中看到的数据，一直跟这个事务启动时看到的数据是一致的
**串行化S(Serializable)**: 对记录加上读写锁，多个事务对这条记录进行读写操作时，如果发生了读写冲突的时候，后访问的事务必须等前一个事务执行完成，才能继续执行；
