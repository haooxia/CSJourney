...
- [Redis八股](#redis八股)
  ...
  - [Redis线程模型/网络模型](#redis线程模型网络模型)
    ...
    - [redis网络模型](#redis网络模型)
  - [缓存](#缓存)
    - [MySQL数据库与Redis缓存的数据一致性 ☆☆☆](#mysql数据库与redis缓存的数据一致性-)
    - [缓存穿透](#缓存穿透)
      - [详解布隆过滤器](#详解布隆过滤器)
    - [缓存雪崩](#缓存雪崩)
...
3. **分布式锁**：通过`setnx key value `(set if not exists)命令，可以确保在分布式系统中的互斥访问
   1. 只有当key不存在时，才能设置成功；设置成功则代表拿到了锁，设置失败则代表锁被别人抢走了
      1. setnx缺陷：没有过期时间，需要配合`expire`设置过期时间，但**这是两步，非原子性**，有小概率出现问题
      2. 这个过期时间应该略大于你的业务逻辑执行时间（加点冗余时间），设置过长的话会导致资源长时间被占用，阻塞其他客户端的请求
   2. 改进：`SET key value NX EX 10`: **nx保证互斥，即key不存在时才可以设置**，ex设置ttl。一条命令同时**加锁+过期**
...
5. **限流器(rateLimiter)**：使用`expire`命令，结合`incr`，可实现API的限流功能，防止系统被过度访问。
   1. 请求计数：每次API请求时，使用INCR命令对特定的key(关于API的唯一标识)进行递增操作；（ttl到期之后又来这儿搞个新key，即新的窗口
   2. 设置过期时间：使用EXPIRE命令**为计数key设置一个ttl(固定窗口)**，过期时间取决于限流的时间窗口（例如1秒）。
   3. 检查请求频率：如果请求计数超过设定的阈值（例如每秒100次），则拒绝新的请求或进行排队。
   4. > 这玩意儿实现的是固定窗口限流算法；我项目中使用zset实现了滑动窗口限流算法（`zadd + zremrangebyscore删除过期的 +  zcard计数`
...
> * BitMap底层就是SDS字符串，一个RedisObject，其中指针指向一个SDS；我们知道SDS结构的核心是一个`char []`，bitmap就把8bit的char的每一位都利用起来了，故而可以直接看做是位数组，仅此而已
> * 位操作的时候：会通过bit的offset进行访问和修改

BitMap使用bit位存储信息(0或1)，即适用于**二值状态统计**的场景；很省内存

```sql
SETBIT key offset value # 设置指定偏移量的位为0或1
GETBIT key offset
BITCOUNT key [start end] # 统计指定key中值为1的位的数量，可以指定范围
BITOP operation destkey key [key …] # 对多个key进行位运算（AND、OR、XOR、NOT）
```
...
![picture 10](../images/88a07ce85d74b61fee28c9bd4143f1e21ee2b43234e9ff6155c54d9b9f91f0ef.png){width=70%}

### 单节点Redis和MySQL的QPS

* 如果缓存命中的话，4核心8g内存的配置，redis可以支撑10w的qps
* 如果缓存没有命中的话，4核心8g内存的配置，mysql只能支持5000左右的qps

### 大Key问题

大Key：某个key对应的value值所占内存空间过大，导致redis性能下降、内存不足、主从同步延迟等问题。

**Q: 多大算大key？**
A: 不固定，根据业务场景合理确定阈值，一般来说：
* 对于String类型，通常认为当value的内存超过**10KB**时
* 对于集合类型（Hash、List、Set、ZSET），如果成员数量超过5000个

#### 大key会造成什么问题？

**i.e., 内存, CPU, 网络, 阻塞, 主从延迟**

* **内存占用过高**：大Key可能导致可用内存不足，从而触发内存淘汰策略。在极端情况下，可能导致内存耗尽，redis崩溃
* **性能下降**：对于大Key的CRUD会消耗更多的**CPU时间**和内存资源
* **网络拥塞**：每次获取大key产生的**网络流量较大**，**严重占用带宽**，同时波及其他服务。
  * 例如：一个大key占用空间是1MB，每秒访问1000次，就有1000MB的流量。
* **客户端耗时增加，甚至超时** (因为你一个客户端请求在操作某大key，单线程，所以其他操作都被**阻塞**掉了)
* **主从同步延迟**：由于大Key占用较多内存，同步过程中需要传输大量数据，这会导致主从之间的网络传输延迟增加，进而影响**数据一致性**
* **数据倾斜**：redis集群中，大key数据分片的内存使用率远超其他数据分片，无法使数据分片的内存资源达到均衡

#### 如何解决大key问题？

* **删除大Key**：将不适用Redis能力的数据存至其它存储，并在Redis中使用`unlink`(redis4.0)安全删除大key（**非阻塞，异步删除**）
* **压缩和拆分key**：
  * 当value是string时，比较难拆分，则使用**序列化、压缩算法**将key的大小控制在合理范围内，但是序列化和反序列化都会带来额外的性能消耗
  * 当value是list/set/hash等集合类型时，根据预估的数据规模来进行**分片**，不同的元素计算后分到不同的片（可以起到平衡数据的作用
* **监控Redis的内存水位**：通过监控系统设置合理的**Redis内存报警阈值**进行提醒，例如Redis内存使用率超过70%、Redis的内存在1小时内增长率超过20%等。
...
![picture 33](../images/7f9fbf30d6bad43a4ca5a07a773733dd9bf741175eb80bf8016408fe92511166.png)  
...
* redis采用**惰性删除+定期删除**，以求合理使用cpu和避免内存浪费
* 惰性删除：redis并**不是当ttl到期就立刻删除**，而是在访问一个key时，检查其tll，如果过期才删除。如果一直不访问，一直不删除...
  * 为了避免对所有key进行定期扫描，节省cpu
* 周期删除：通过一个定时任务，每隔一段时间秒**抽样20个key进行检查，然后删除其中的过期key**（默认**100ms**
  * 如果这20个key中过期key超过25%，则接着抽20个来删除，**循环过程**
  * 但为了不一直循环过度，导致线程卡死，循环时间设置不超过**上限25ms**（疯狂打补丁是吧

## 集群

![picture 35](../images/78b41b0d01dde33d747b5129e724d2e08d2a03aece12668c5241955e830e98a5.png)  
...
1. **故障恢复**：如果主节点挂掉了，可以将一个从节点提升为主节点，从而实现故障的快速恢复
2. **负载均衡**：在主从复制的基础上，配合读写分离，分担服务器负载，提高**并发量**
3. **高可用基石**：主从复制还是**哨兵和集群**能够实施的基础。
4. **数据冗余**：实现了数据的**热备份**，是持久化之外的一种数据冗余方式
...
* 如果存的字符串时**整数值**，则采用 ==**INT编码**==，直接就存在RedisObject的ptr指针的位置（刚好8B，你一个Long也就8B，足矣），无需额外的SDS；
  * redis真是内存机制节省大师...
