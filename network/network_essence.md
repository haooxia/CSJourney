...
3. **浏览器和服务器通过三次握手建立TCP连接（由os网络协议栈调用socket完成）**。(在https请求中，浏览器还会通过**SSL/TLS**协议进行加密)。连接建立后，浏览器端会构建请求行、消息头和消息体，向服务器**发送请求报文**。
...
* PUT：**替换指定的资源**。如果指定的资源不存在，创建一个新资源。不同于POST，PUT是**幂等**的
...
1. **作用**不同（使用场景）
   1. GET用于从服务端获取资源或数据，适用于**查询操作**
   2. POST一般用来向服务器发送数据来**提交或更新**资源，适用于**表单提交、用户注册登录、文件上传**等场景
2. **参数传递方式**不同
...
3. **缓存机制**不同
   1. GET请求会被浏览器主动缓存，也可以添加到书签中。而POST一般不会
<!-- 4. 时间消耗不同
   1. GET 产生一个 TCP 数据包；
   2. POST 产生两个 TCP 数据包。
...
- 区别
  - 是否**面向连接**
  - 是否是**可靠**传输：TCP是可靠的传输服务，在传递数据之前，会有三次握手来建立连接；在数据传递时，通过**序列号、确认应答和重传机制**来保证数据完整、无误、**有序**地到达接收方。UDP是不可靠传输(尽最大努力交付)，数据可能丢失、重复或**乱序**接受。（也可以基于QUIC协议整个可靠的UDP传输）
...
* **确认应答与超时重传**：TCP给发送的每个包指定序列号，接收方会对收到的包进行应答，发送方就会知道接收方是否收到对应的包；如果指定时间后，发送端仍未收到确认应答，说明出现==丢包问题==，则启动**超时重传**
...
* **应用层ACK确认机制**：发送方在发送数据后，要求接收方返回一个确认消息（ACK），只有收到ACK后才认为数据成功传输，否则进行重传
  * 使用场景：消息队列、文件传输..
* **手动设置超时重传**：发送方在一定时间内未收到ACK，则重新发送数据包。
  * 通过超时计时器
* **进行数据包编号**：在数据包中添加序列号，接收方可以根据序列号判断数据是否丢失、重复或乱序
* 直接使用**可靠UDP协议**：
...
1. 客户端发送FIN报文，报文中指定一个序列号(seq=x)，然后客户端进入`fin_wait_1`状态。表示客户端没数据发送了，但还可以接收数据。
