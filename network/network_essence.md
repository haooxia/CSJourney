...
* **序列号机制**：发送端将数据分割为报文段，每个段对应唯一序号，接收端可以根据序号**重组乱序**到达的段；同时，接收端可以根据序号**去重**，避免重复接收
* **错误检测**：使用**校验和**（首部+数据）检测数据在传输过程中的损坏，损坏则丢弃
* **确认应答**：TCP给发送的每个包指定序列号，接收方会对收到的包进行应答，发送方就会知道接收方是否收到对应的包；如果指定时间后，发送端仍未收到确认应答，则启动**超时重传**
